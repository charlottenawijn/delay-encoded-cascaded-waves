% Example script to transmit a delay-encoded pulse train with a GE C1-6-D
% ultrasound probe and Verasonics Vantage 256 research ultrasound system.
% The programmed pulse train contains 7 pulses, within a sacrificial depth
% of 15 mm. This script is an adapted example script provided by
% Verasonics.

% Charlotte Nawijn, University of Twente, 2024

% Decoding after acquisition should be performed before beamforming, and
% consists of dividing the RF lines by coding function C, 
% which is based on the pulse delays:
% C = sum(exp(-1i * P.pulse_delays .* omega), 2);
% with: 
% P.pulse_delays = P.delay_samples / P.f_sampling_receive;
% f = linspace(0, P.f_sampling_receive - 1/(max(duration_RF) - min(duration_RF)), n);
% omega = 2*pi.*f;
% where 'n' and 'duration_RF' are the total length/duration of the acquired 
% RF lines (in samples and seconds, respectively)
...
% interpulse delays (samples) for N=7 for sacrificial depth 15 mm
P.delay_samples = [0, 153, 292, 440, 587, 744, 886];
P.f_sampling = 250e6;               % sampling frequency Verasonics Vantage 256 system
P.f_sampling_receive = 250e6/4;     % sampling frequency of aquisition

% 'time' between waves is the time we want from the start of the last
% pulse to the current one, minus the time (in samples) it took
% Verasonics to transmit the previous pulse (here in samples)
P.time_between_waves_temp = zeros(1, length(P.delay_samples));
for p = 2: length(P.delay_samples)
    P.time_between_waves_temp(p) = (P.delay_samples(p) - P.delay_samples(p-1)) ./ P.f_sampling_receive; 
end
P.time_between_waves = P.time_between_waves_temp * P.f_sampling;   
P.time_between_waves(2:end) = P.time_between_waves(2:end) - (21+32+42+21+42+32+21);     % see PulseCode below

% create pulse train with TW.type = 'pulseCode'
% based on TW.States, where the first column denotes whether the
% element is on (+1/-1) or off (0), and the second column denotes
% the duration in clock periods at 250 MHz (T=0.004 us).
% For pulseCode: [Z1 P1 Z2 P2 R], where Z1 is the
% duration of 0 state, P1 the duration of on state (sign
% determines polarity), Z2 is the duration of 0 state between P1
% and P2, P2 is the duration of the on state again, and R is the
% number of times [Z1 P1 Z2 P2] is repeated
TW(1).type = 'pulseCode';

% short imaging pulse for N = 1
TW(1).PulseCode = [5 -21 32 42 1; 21 -42 32 21 1];

% delay-encoded pulse train
for i = 2: length(P.delay_samples)
    TW(1).PulseCode = [TW(1).PulseCode; ...
        P.time_between_waves(i) -21 32 42 1; 21 -42 32 21 1];
end
...
save('MatFiles/GEC1-6DFlash_DE_CW');
